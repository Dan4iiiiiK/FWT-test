...
function Picture({imageUrl, locationId, created, authorId, name}: IPicture) {
  ...
  const authorsName = authors?.map((authors) => authors.id === authorId ? authors.name.toUpperCase() : '')
  
  const locationsName = location?.map((location) => location.id === locationId ? location.location.toUpperCase() : '')

  return (
        <li className={styles.picture__box}>
          <img src={`https://test-front.framework.team${imageUrl}`} alt={name} className={styles.picture__img}/>
          <div className={styles.picture__textArea}>
            ...
            <div className={`${styles.picture__text} ${styles.picture__text__hover}`}>
